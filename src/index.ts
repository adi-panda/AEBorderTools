...
const recursiveBorderCreation = (
  ...
  var borderPosition =
    newLayer.property("Contents")(1)("Transform")("Position");
  ...
  if (borderPosition instanceof Property) {
    ...
    baseLayerDuplicate.position.setValue([
      baseLayerDuplicate.position.value[0] - borderPosition.value[0],
    ...
    app.setTimeout(function () {
      ...
      if (borderPath instanceof Property) {
        for (
          ...
          pointNames.push(
            `fromComp(comp("${comp.name}").layer("🔥${newLayer.name}").effect("Pseudo/YourCustomControl_v1")("Point ${originalPointCount}").value)`
          );
        }
      ...
      var newComp = comp.layers.precompose(
        [newLayer.index, borderDuplicate.index, baseLayerDuplicate.index],
        "🔥" + newLayer.name,
        true
      );
      ...
      comp.layers[currentLayerIndex].position.setValue([
        oldPosition[0] + xTransformDistance,
        oldPosition[1] + yTransformDistance,
      ]);

      applyBorderPreset(
        comp.layers[currentLayerIndex],
        "Border Controller",
      ...
      for (
        ...
        var pointVal = comp.layers[currentLayerIndex].property(
          "ADBE Effect Parade"
        )("Pseudo/YourCustomControl_v1")("Point " + originalPointCount);
        if (pointVal instanceof Property) {
          pointVal.setValue(
            selectedShapeLayer.sourcePointToComp(
              originalPointVertices[originalPointCount - 1]
            )
          );
          originalPointVertices[originalPointCount - 1] =
            selectedShapeLayer.sourcePointToComp(
